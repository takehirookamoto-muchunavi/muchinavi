...
**各ステップ完了後、ユーザーに確認してから次に進んでください。**
特にStep 2（設計）とStep 5（最終修正）後は必ず承認を取ること。
...
**>>> ユーザー確認ポイント: 設計書（KW・構成案）を提示し、OKをもらってから次へ <<<**
...
**>>> ユーザー確認ポイント: v2記事の方向性・トーン・CTA方針を確認してから次へ <<<**
...
### 認証情報の設定方法（初回のみ）
WordPress Application Passwordがまだない場合:
1. muchinochi55.com/wp-admin → ユーザー → プロフィール
2. 「アプリケーションパスワード」セクションで新しいパスワードを生成
3. ユーザー名とパスワードをBase64エンコード: `echo -n "username:app_password" | base64`
4. 結果のBase64文字列をメモしておく（次回以降も使用）
