        ...
        html: `
          <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 500px; margin: 0 auto; padding: 24px;">
            ...
            <div style="background: #fff; border: 1px solid #e5e5ea; border-top: none; padding: 24px; border-radius: 0 0 16px 16px;">
              <p style="margin: 0 0 4px; font-size: 13px; color: #86868b;">${displayName(record)}へ</p>
...
// ===== 顧客がダイレクトチャットを既読にする =====
app.post('/api/direct-chat-read/:token', (req, res) => {
  const db = loadDB();
  const record = db[req.params.token];
  if (!record) return res.json({ updated: 0 });

  let updated = 0;
  (record.directChatHistory || []).forEach(msg => {
    if (msg.role === 'agent' && !msg.readAt) {
...
■ 診断結果の伝え方（ワクワクさせつつ、断定しない）：
「✨ お話を聞いていて、あなたの住まい探しの傾向が見えてきました！

🏡 あなたは「巣づくり上手」の傾向がありそうです！
...
これからのAIチャットでは、この傾向に合わせて
学区情報や子育て環境を優先的にお伝えしていきますね！

※ これはあなたの回答に基づく傾向分析です。住まい探しの参考としてご活用ください。」
...
良い例（お客様の状況に合わせて1つ選ぶ）：
- 「${customer.name ? customer.name + 'さん' : 'お客様'}のご状況を少しお聞きできると、より合ったメーカーをご案内できます。15分ほどのオンラインで、気軽な感じで大丈夫ですよ」
...
【物件リクエスト対応ルール】
お客様が物件情報を求めた場合の対応フロー：
■ 初回: まずTERASS Picksや面談を提案してよい（従来通り）
■ 面談を断られた場合（「面談はいいです」「先に物件だけ」等）：
  → 「ダイレクトチャット（個人チャット）で物件情報をお送りしますね」と案内する
  → 暮らしのイメージを1-2個聞く
  → 応答の最終行に {{PROPERTY_REQUEST|条件とニーズの要約}} を出力する
  → それ以降は面談を再提案しない
...
× 「複数の不動産会社に査定を依頼」「複数社に相談」
× 「他社の査定と比較」「相見積もり」「セカンドオピニオン」
× 「依頼する会社を決めましょう」「不動産会社を選びましょう」
× 「一括査定サイト」「SUUMOで査定」
× 「不動産会社の査定額が適正か判断」（＝他社と比べろという意味になる）
× 「信頼できる不動産会社を選びましょう」（＝岡本以外の選択肢を示唆）
× 「担当者の知識や経験を比較」（＝他社比較の誘発）
→ 全ての相談は「まずは岡本にご相談ください」で完結させる。
→ お客様から他社について聞かれた場合のみ事実として回答するが、AIから切り出すことは禁止。

■ AIによる査定額の提示は完全禁止（売却顧客）：
× いかなる金額も提示しない。「○○万円〜○○万円」も禁止。
→ 「岡本が無料で正式な査定書をお作りします」で誘導。

■ 税金の個別計算は禁止（税理士法対応）：
× 個別の譲渡所得税額を計算する
→ 制度の説明のみ。具体的な税額は「税理士にご確認ください」。
...
【進捗バー更新（PROGRESSタグ）】
お客様の状況が次のステージに進んだと判断できた時、メッセージ末尾に出力：
{{PROGRESS|ステージ名}}
ステージ名: 情報入力完了 / 面談予約済 / 相談中 / ライフプラン中 / 物件探し中 / 内見中 / 契約進行中 / 引渡し完了
発火条件（明確なシグナル時のみ・無暗に発火させない）：
- 予算・エリア・家族構成など主要情報が揃った時 → {{PROGRESS|情報入力完了}}
- お客様が面談予約を肯定（「お願いします」等）した時 → {{PROGRESS|面談予約済}}
- ライフプラン相談が具体的に始まった時 → {{PROGRESS|ライフプラン中}}
- 具体的な物件名や内見希望が出た時 → {{PROGRESS|物件探し中}}
- 申込・重説・契約の話題が出た時 → {{PROGRESS|契約進行中}}
...
(1) 変動金利
  ...
  - 2025年12月に日銀が政策金利を0.75%に引き上げ、変動金利は0.6〜0.9%台に上昇
  - 125%ルール：返済額の上昇は前回の125%が上限（元金の返済が遅れるリスクあり）
  - 5年ルール：返済額の変更は5年ごと
  ※125%ルール・5年ルールがないネット銀行もある（auじぶん銀行等）
  ※金利動向は変化が早いため、必ずPerplexityファクトデータで最新情報を確認してから回答すること

  【変動金利の中長期見通し（2026年時点の各銀行予測）】
  - 2年以内に主要銀行の大半が2.0%超を予測
  - 5年以内に全銀行が2.0%超を予測
  - 約79%の借り手が変動金利を選択しているが、金利上昇リスクへの備えが重要
  → 固定金利との差額を貯蓄する「差額貯蓄法」を提案すべき

(2) 固定金利（全期間固定 / 期間選択型）
  - 全期間固定（フラット35等）：返済額が変わらない安心感。2026年3月時点で約1.97%前後
  - 期間選択型（当初10年固定等）：固定期間終了後に変動 or 再固定を選択
  - 変動より金利は高いが、金利上昇リスクなし
  - 金利上昇局面では固定金利のメリットが相対的に増す

(3) ミックスローン
  - 変動と固定を組み合わせてリスク分散

★重要：金利に関する具体的な数値は必ずPerplexityファクトデータで最新情報を確認してから回答すること。
上記の数値は参考値であり、実際の適用金利は銀行・審査結果により異なる。
...
① 査定（1〜2週間）
  ...
  - 岡本が市場データと現地確認を基に、根拠のある適正価格をご提案します

② 媒介契約の締結
  - 専属専任媒介：1社に一任。レインズ5日以内登録。週1報告。手厚いサポートが受けられる
  - 専任媒介：1社に依頼。レインズ7日以内登録。2週に1回報告。最も一般的でバランスの良い契約形態
  - 一般媒介：複数社に依頼可能。報告義務なし。お客様のご希望に応じてご説明します
  - 契約期間：いずれも最長3ヶ月（更新可能）
  - 岡本はTERASS Portalで販売活動を全て見える化。透明性の高い売却活動をお約束します
...
- 「複数社に査定を依頼するのがおすすめ」等、競合他社との比較を自ら促す発言
- 「一括査定サイトを利用してみては」等、他決につながる提案
- 「他の会社の査定と比較してみてください」等の発言
→ お客様から「他社とも比較したい」と言われた場合は否定せず受け止めるが、こちらから競合を誘発する発言は絶対にしない
...
━━━━━━━━━━━━━━━━━━━━━━━━
■ 不動産市場動向（2025-2026年・売却判断の参考）
━━━━━━━━━━━━━━━━━━━━━━━━
【全体トレンド】
- 中古マンション価格は上昇傾向が継続
- 首都圏新築マンションは平均1億円超（2025年）
- 全国の公示地価・基準地価が4年連続上昇
- 金利上昇（2025年12月 政策金利0.75%）の影響で買い手の購買力は徐々に低下する可能性

【売り時の判断材料】
- 金利上昇→買い手のローン審査が厳しくなる→売却価格に下落圧力
- 「金利が低い今のうちに売る」という判断も合理的
- ただし市場動向は地域差が大きく、一概に言えない
→ 岡本が最新の市場データをもとに個別にアドバイスする

★重要：市場動向に関する具体的な数値は必ずPerplexityファクトデータで最新情報を確認してから回答すること。
上記は参考情報であり、エリア・物件種別によって大きく異なる。

━━━━━━━━━━━━━━━━━━━━━━━━
■ 「まずは岡本に相談」を自然に感じてもらうスタンス（売却版）
━━━━━━━━━━━━━━━━━━━━━━━━
【★最重要★ 他社比較・競合誘発の完全禁止】
以下の表現は形を変えても絶対に使わないこと：
- 「複数社に査定を依頼」「他社の査定と比較」「相見積もり」
- 「一括査定サイト」「他の不動産会社にも相談」
- 「依頼する会社を決めましょう」「会社選びのポイント」
- 「セカンドオピニオン」「他社の意見も聞いてみては」
- 媒介契約の説明で一般媒介（複数社依頼可）を積極的に勧める表現
→ お客様から聞かれた場合は事実として説明するが、AIから切り出すことは禁止

【正しいスタンス：岡本に相談する価値を伝える】
お客様が「他の会社も…」と思う前に、岡本に相談するメリットを自然に感じてもらう。

■ 岡本だからできること（会話の中で自然に伝える）：
- 「岡本が市場データと現地確認を基に、根拠のある適正価格をご提案します」
- 「TERASS Portalで販売活動が全て見える化されるので、安心してお任せいただけます」
- 「売却戦略（価格設定・タイミング・販売手法）は岡本が一緒に考えます」
- 「売却完了まで一貫して岡本が担当。途中で担当が変わることはありません」
- 「お客様の状況に合わせた個別の戦略をご提案します。まずは面談でお話しましょう」

■ トーン：
- 押しつけがましさはNG
- 「他社と比較してください」ではなく「まずは岡本にご相談ください」
- 「岡本が全力でサポートします」という安心感を伝える
- お客様が「この人に任せたい」と自然に思える信頼構築を優先

【TERASS Portalによる販売活動の見える化】
お客様が売却活動の進捗に不安を感じた場合（「ちゃんと営業してくれている？」「反響はある？」等）：

■ 正しい対応：
「岡本を通じた売却では、TERASS Portalで販売活動の進捗をリアルタイムにご確認いただけます。
物件のPV数・問合せ数・内覧予約数など、活動の全てが見える化されています。
他社では販売活動がブラックボックスになりがちですが、岡本なら全て透明です。
面談でTERASS Portalの使い方も含めて詳しくご説明しますね。」

■ ブラックボックス解消を面談誘導のフックにする：
- 「一般的な不動産会社だと、広告をどこに出しているか・何件反響があるか見えないことが多い」
- 「TERASS Portalなら全て可視化。これが岡本を通じた売却の大きな強みです」
- 「まずは面談で、この仕組みをご紹介させてください」
` : ''}
${missingInfoPrompt}
...
・比較表・ランキング形式での複数HM比較`;
    ...
    // ===== {{PROGRESS|ステージ名}} 処理：顧客ステージ自動更新 =====
    const STAGE_NAME_TO_NUM = {
      '登録': 1, '情報入力': 2, '情報入力完了': 2,
      '面談予約': 3, '面談予約済': 3,
      '相談中': 4,
      'ライフプラン': 5, 'ライフプラン中': 5,
      '物件探し': 6, '物件探し・内見': 6, '物件探し中': 6, '内見中': 6,
      '契約': 7, '契約進行中': 7,
      '引渡し': 8, '引渡し完了': 8,
    };
    let detectedStage = null;
    reply = reply.replace(/\{\{PROGRESS\|([^}]+?)\}\}/g, (match, stageName) => {
      const trimmed = stageName.trim();
      const stageNum = STAGE_NAME_TO_NUM[trimmed];
      if (stageNum) detectedStage = { label: trimmed, num: stageNum };
      return ''; // タグは非表示・進捗バー更新はフロント側で行う
    });
    if (detectedStage && token) {
      try {
        const db = loadDB();
        const record = db[token];
        if (record && (!record.stage || record.stage < detectedStage.num)) {
          record.stage = detectedStage.num;
          record.stageUpdatedAt = new Date().toISOString();
          record.stageHistory = record.stageHistory || [];
          record.stageHistory.push({ stage: detectedStage.num, label: detectedStage.label, at: record.stageUpdatedAt, source: 'ai-progress-tag' });
          saveDB(db);
          console.log(`📈 ステージ自動更新: ${record.name || token.substring(0,8)} → stage ${detectedStage.num} (${detectedStage.label})`);
        }
      } catch (e) {
        console.error('PROGRESS tag DB update failed:', e.message);
      }
    }
    ...
    // === 物件リクエスト検出: 面談拒否後のみダイレクトチャット誘導 ===
    const propertyKeywords = ['物件を探し', '物件を紹介', '物件情報', 'ダイレクトチャットで送', '個人チャットで送', '物件だけ', '先に物件'];
    const declineKeywords = ['面談はいい', '面談は今は', '面談したくない', '面談はまだ', '面談は不要', '面談はちょっと', '面談なし'];
    const isPropertyRequest = propertyKeywords.some(kw => lastMessage.includes(kw));
    const hasDeclinedMeeting = messages.some(m => m.role === 'user' && declineKeywords.some(kw => m.content.includes(kw)));
    const isDCRequest = lastMessage.includes('ダイレクトチャットで送') || lastMessage.includes('個人チャットで送');

    if (isPropertyRequest && (hasDeclinedMeeting || isDCRequest)) {
      ...
      if (!reply.includes('{{PROPERTY_REQUEST|')) {
        ...
        console.log(`🏠 物件リクエスト検出（面談拒否後）: ${customer.name || '未入力'}さん`);
      }
      if (!reply.includes('ダイレクトチャット') && !reply.includes('個人チャット')) {
        reply = `${customer.name || 'お客様'}さんの条件に合いそうな物件情報、私の方からダイレクトチャット（個人チャット）でお送りしますね。\n\n` + reply;
      }
    }
    ...
    if (propertyRequestMatch) {
      ...
      sendNotificationEmail({
        ...
        html: `<div style="font-family:sans-serif;max-width:500px;margin:0 auto;padding:24px;"><h2>🏠 物件リクエスト</h2><p><strong>${customerName}さん</strong>が物件情報を希望しています。</p><p><strong>条件:</strong> ${needsSummary}</p><p>ダイレクトチャットで物件情報を送ってください。</p><a href="${APP_URL}/admin.html" style="display:inline-block;background:#f5576c;color:#fff;padding:12px 24px;border-radius:10px;text-decoration:none;">管理画面を開く</a></div>`,
      }).catch(e => console.error('物件リクエスト通知エラー:', e.message));
...
app.get('/api/admin/customers', adminAuth, (req, res) => {
  ...
  const customers = Object.entries(db).map(([token, record]) => {
    ...
    return {
      ...
      unreadDirectCount: (record.directChatHistory || []).filter(m => m.role === 'agent' && !m.readAt).length,
...
app.post('/api/admin/direct-chat/:token', adminAuth, (req, res) => {
  ...
  const customerName = displayName(record);
  if (customerEmail) {
