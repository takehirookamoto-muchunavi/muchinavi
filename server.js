...
■ 面談を断られた場合（「面談はいいです」「先に物件だけ」等）：
  ...
  → それ以降は面談を再提案しない。突き放さず「味方であり続ける」姿勢を貫く
...
【物件リクエスト対応 — 面談拒否後のフォールバック】
お客様が物件情報を求め、かつ面談を断った場合（「面談はいいです」「先に物件だけ見たい」等）：

■ 対応フロー：
1. 「○○さんの条件に合いそうな物件情報、私の方からダイレクトチャット（個人チャット）でお送りしますね」と伝える
2. 暮らしのイメージ（潜在ニーズ）を1-2個自然に聞く
3. 応答の最終行に {{PROPERTY_REQUEST|ニーズ要約}} を出力する
4. 面談の再提案はしない

■ 初回の物件リクエスト（面談を断っていない段階）：
→ TERASS Picksの案内や面談提案は従来通りOK
...
■ 進捗バー更新（PROGRESSタグ）：
お客様の状況が次のステージに進んだと判断できた時、メッセージ末尾に以下を出力：
{{PROGRESS|ステージ名}}
ステージ名は次のいずれか（お客様の状態に応じて選択）：
・情報入力完了 ／ 面談予約済 ／ 相談中 ／ ライフプラン中 ／ 物件探し中 ／ 内見中 ／ 契約進行中 ／ 引渡し完了
発火条件（明確なシグナル時のみ）：
...
【最終リマインダー】
■ 初回の物件リクエスト → TERASS Picksや面談を提案してよい（従来通り）
■ 面談を断られた後の物件リクエスト → 「ダイレクトチャットでお送りします」と案内し、{{PROPERTY_REQUEST|ニーズ要約}} を出力。面談の再提案はしない`;
    ...
    reply = reply.replace(/\{\{PROGRESS\|([^}]+?)\}\}/g, (match, stageName) => {
      ...
      return ''; // タグ自体は非表示（進捗バーの更新はフロント側で行う）
    });
    // ステージが進んだ場合のみ DB を更新
    ...
    if (isPropertyRequest && (hasDeclinedMeeting || isDCRequest)) {
      ...
      if (!reply.includes('{{PROPERTY_REQUEST|')) {
        ...
        console.log(`🏠 物件リクエスト検出（面談拒否後）: ${customer.name || '未入力'}さん → メール通知送信`);
      }
